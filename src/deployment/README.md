...
Status: Deployment configuration area; current compose runtime remains in `src/production`

`src/deployment` is for deployment configuration after runtime source is separated from deployment variants.

## Current Compatibility Boundary

- Authoritative local runtime compose files remain in `src/production/docker-compose.yml` and `src/production/docker-compose.test.yml`.
...
- HMI deployment-specific frontend configuration should be reconciled with `src/production/HMI/ui`, not maintained as a second frontend tree.
