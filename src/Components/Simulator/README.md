...
The simulator is part of the production `src/Components` boundary, but it is Engine-owned because it produces testable animal/audio events for the Engine runtime. It publishes simulated recordings to MQTT and listens for simulator control messages.
...
| `Simulator.Dockerfile` | Docker image used by `src/Components/docker-compose.yml`. | Active. |
