...
Owner: IoT team
Status: Mixed production and review area inside the current `src/Components` runtime boundary
...
- `src/Components/docker-compose.yml` builds `echo_iot_management` from `./IoT/management_application/Dockerfile`.
...
| File | Lines | Finding |
| --- | --- | --- |
| `src/Components/IoT/management_application/client_pi.py` | 15 | Upload endpoint is hard-coded to LAN URL `http://192.168.1.122:5000/upload`; this does not clearly match EchoNet API routing. |
| `src/Components/IoT/previous_implementation/client_pi.py` | 15 | Same hard-coded LAN upload endpoint as management application. |
| `src/Components/IoT/previous_implementation/config/devices.json` | 4, 10 | Device records contain fixed LAN IPs `192.168.1.122` and `192.168.1.123`. |
| `src/Components/IoT/edge_inference/iot_edge_client.py` | 218 | MQTT broker defaults to public broker `broker.hivemq.com`. |
| `src/Components/IoT/edge_inference/README.md` | 151, 153, 175, 177, 217, 228 | Documentation examples and notes use `broker.hivemq.com` / `iot/data/test`. |
| `src/Components/IoT/2025_T3_prototype/client.py` | 13 | MQTT broker hard-coded to `broker.hivemq.com`. |
| `src/Components/IoT/2025_T3_prototype/server.py` | 9 | MQTT broker hard-coded to `broker.hivemq.com`. |
| `src/Components/IoT/2026_T1_new_onboarding/onboarding_client.py` | 12, 42 | Public broker `broker.hivemq.com`; fallback IP uses `127.0.0.1`. |
| `src/Components/IoT/2026_T1_new_onboarding/onboarding_server.py` | 6 | HiveMQ Cloud broker hostname is hard-coded. |
| `src/Components/IoT/previous_implementation/setup/onboarding_client.py` | 9, 37 | HiveMQ Cloud broker hostname and `127.0.0.1` fallback are hard-coded. |
| `src/Components/IoT/previous_implementation/setup/onboarding_server.py` | 5 | HiveMQ Cloud broker hostname is hard-coded. |
