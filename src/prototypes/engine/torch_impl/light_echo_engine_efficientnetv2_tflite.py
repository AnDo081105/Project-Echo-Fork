...
_ENV_CONFIG_TYPES = {
    "AUDIO_DATA_DIRECTORY": str,
    "CACHE_DIRETORY": str,
    "MQTT_CLIENT_URL": str,
    "MQTT_CLIENT_PORT": int,
    "MQTT_PUBLISH_URL": str,
    "GCLOUD_PROJECT": str,
    "BUCKET_NAME": str,
    "MODEL_SERVER": str,
    "WEATHER_SERVER": str,
...
def apply_environment_overrides(config):
    for key, caster in _ENV_CONFIG_TYPES.items():
        value = os.getenv(key)
        if value is None:
            continue
        try:
            ...
            print(f"Ignoring invalid environment override for {key}: {value}", flush=True)
    return config
